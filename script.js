...
function getProjectItems(projectJsonLd) {
    const listItems = projectJsonLd?.mainEntity?.itemListElement || [];
    return listItems.map(listItem => listItem.item || listItem);
}
function projectFromJsonLd(item) {
    const links = item.links || [];
    return {
        name: item.name,
        category: item.uiCategory || item.genre?.[0] || "",
        featured: Boolean(item.featured),
        imgPath: item.localImage || item.image,
        mobileImgPath: item.mobileImage || item.thumbnailUrl,
        imgAlt: item.imageAlt || `${item.name} project preview`,
        imgLink: item.url,
        description: item.description,
        features: item.featureList || item.keywords || [],
        pLinks: links.map(link => [link.url, link.icon || "", link.label || link.name || "View project"])
    };
}
async function getProjects() {
    ...
    if (projectsJsonLdScript) {
        projectsJsonLdScript.textContent = JSON.stringify(projectJsonLd, null, 2);
    }
    return getProjectItems(projectJsonLd).map(projectFromJsonLd);
}
async function buildProjects(projects) {
    ...
    projects.forEach((proj, index) => {
        ...
        const mobileSource = clone.querySelector(".project-mobile-source");
        const imgLink = clone.querySelector(".p_img-link");
        const description = clone.querySelector(".project-desc");
        const featureList = clone.querySelector(".project-features");
        const linkList = clone.querySelector(".project-links");
        const pLinkItems = proj.pLinks || [];
        title.textContent = proj.name;
        title.dataset.i18n = `projects.${index}.title`;
        if (proj.featured)
            clone.querySelector(".project").classList.add("project-featured");
        category.textContent = proj.category || "";
        category.dataset.i18n = `projects.${index}.category`;
        img.src = proj.imgPath || `images/icons/projects/${proj.img}`;
        if (proj.mobileImgPath) {
            mobileSource.srcset = proj.mobileImgPath;
        }
        else {
            mobileSource.remove();
        }
        img.alt = proj.imgAlt || `${proj.name} project preview`;
        ...
        if (proj.features && proj.features.length > 0) {
            proj.features.forEach((feature, featureIndex) => {
                const li = document.createElement("li");
                li.textContent = feature;
                li.dataset.i18n = `projects.${index}.features.${featureIndex}`;
                li.dataset.i18nDefault = feature;
        ...
        pLinkItems.forEach(links => {
            ...
            label.classList.add('p_link-label');
            a.href = links[0];
            a.target = "_blank";
            ...
            if (links[2]) {
                label.textContent = links[2];
                label.dataset.i18n = `projects.${index}.links.${linkList.children.length}.label`;
                label.dataset.i18nDefault = links[2];
                a.append(label);
                a.classList.add("p_link-text");
            }
            li.append(a);
            linkList.append(li);
        });
        ...
        category.dataset.i18nDefault = proj.category || "";
...
expSection.addEventListener("click", (e) => {
    ...
    if (!list || !list.classList.contains("duties-list"))
        return;
...
async function loadContent(filePath) {
    ...
    if (!ifEmpty(projectsSection, projects)) {
        ...
        document.querySelectorAll('a[href="#projects"]').forEach(link => link.closest("li")?.remove());
    }
